...
async function init () {
  sa.get('https://discord.com/api/gateway/bot').set('Authorization', `Bot ${process.env.BOT_TOKEN}`).then(async b => {
    ...
    global.logger.startup(`[SHARDER]: Splitting ${totalShards} total shards across ${coreCount} cores and ${workerCount} workers means ${shardsPerWorker} Shards per worker ${process.env.USE_MAX_CONCURRENCY === 'true' ? '\nMax concurrency is enabled.' : ''}`)
