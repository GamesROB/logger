...
module.exports = {
  func: async (message, suffix) => {
    if (!message.member.permissions.json.manageGuild) {
      ...
      await message.channel.createMessage({
        embed: {
          description: 'The provided timezone is not valid. You can use any timezone [here](https://github.com/GamesROB/supportbot/blob/development/timezones.md) or `reset` to clear the saved timezone.',
          author: {
            name: `${message.author.username}#${message.author.discriminator}`,
            icon_url: message.author.avatarURL
          },
  ...
  name: 'settz',
  quickHelp: 'Set the timezone the bot uses in your logs. Usable timezones are [here](https://github.com/GamesROB/supportbot/blob/development/timezones.md). Custom timezones affect the userinfo command as well as channelDelete, channelUpdate, guildMemberAdd, guildMemberRemove, and messageDelete events.',
